  ...
  ASSERT_THROW(
    {
      try
        ...
        {
          // Verify the error message mentions duplicate qualified_id
          std::string error_msg = e.what();
          EXPECT_TRUE(error_msg.find("Duplicate qualified_id") !=
                      std::string::npos)
            << "Error message should mention duplicate qualified_id. Got: "
            << error_msg;
