...
main(int argc, char *argv[])
{
    ...
    CLI::App* register_sub = app.add_subcommand(
        "register",
        "Register node type"
    );

    fs::path register_path{"node_types.json"};
    register_sub->add_option(
        "register_path",
        register_path,
    ...
    run_sub->add_option(
        "input_json",
        input_json,
        "Input json of the graph to run"
    )->required()
    ...
    if (dump_reg) {
        ...
        std::cout << "Dumped registered node to " << register_path << "." << std::endl;
    }
